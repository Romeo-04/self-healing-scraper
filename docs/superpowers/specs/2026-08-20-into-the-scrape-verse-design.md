...
Real output from collector the project collector confirms the shape: `title`, `price: { value, currency, symbol }`, `availability`, `product_url`, `product_page_url`. Hence `path` reaching `price.value`.
...
| 3 | `BRIGHT_DATA_COLLECTOR_ID` | **done** — value lives in `.env.local`, never committed |
...
2. **Can a collector return raw page HTML alongside parsed fields?** — **answered 2026-08-21: no.** A real run of the project collector returned only parsed JSON (`title`, `price: { value, currency, symbol }`, `availability`, `product_url`, `product_page_url`) with no HTML field and no markup anywhere in the payload.
